...
const contracts = [
  ...
  },
  {
    ...
    path: 'CompoundStrategy.sol/CompoundStrategy.json'
  },
  {
    name: 'LiquidStakingStrategy',
    path: 'LiquidStakingStrategy.sol/LiquidStakingStrategy.json'
  },
  {
    name: 'LiquidityProvidingStrategy',
    path: 'LiquidityProvidingStrategy.sol/LiquidityProvidingStrategy.json'
  },
  {
    name: 'StrategyManager',
    path: 'StrategyManager.sol/StrategyManager.json'
  },
  {
    name: 'MockERC20',
...
// Create JavaScript module for easy importing
const jsModule = `// Auto-generated contract exports
const contracts = ${JSON.stringify(exportedContracts, null, 2)};

module.exports = contracts;

// Individual exports for convenience
${Object.keys(exportedContracts).map(name => 
  `module.exports.${name} = contracts.${name};`
).join('\n')}
`;

fs.writeFileSync(path.join(exportsDir, 'index.js'), jsModule);

console.log(`\n🎉 Successfully exported ${Object.keys(exportedContracts).length} contracts to exports/`);
console.log('📁 Files created:');
console.log('  - exports/index.json (combined JSON)');
console.log('  - exports/index.js (JavaScript module)');
Object.keys(exportedContracts).forEach(name => {
  console.log(`  - exports/${name}.json`);
});
